# Documento de Referência: Banco de Dados MATA60
...
*   **Justificativa/Parâmetros:** Inclui nome, e-mail e um tipo de vínculo (`TP_VINCULO_INST` ex: 'ALUNO'). Isso permite segmentar os tipos de público que a universidade/instituição está atendendo.
...
*   **Justificativa/Parâmetros:** Une `TB_PARTICIPANTE` a `TB_ATIVIDADE`. Possui restrições (`CHECK`) importantes: presença só pode ser 'PRESENTE' ou 'AUSENTE', e a nota (`VL_NOTA_AVALIACAO`) deve estar entre 0 e 10. Estas travas garantem a integridade das regras de negócio.
...
*   **Justificativa/Parâmetros:** Utiliza chave primária composta (ID_ATIVIDADE e ID_INSTRUTOR) para evitar duplicidade na alocação, e também registra a carga horária empenhada pelo instrutor na atividade.

**10. `RL_PATROCINIO_EVENTO`**
*   **Descrição:** Relaciona parceiros e atividades com o montante de patrocínio.
*   **Justificativa/Parâmetros:** Chave primária composta. Adiciona a coluna financeira `VL_APORTE` permitindo rastrear o investimento exato recebido por cada evento.
...
*   **Justificativa/Montagem:** Utiliza o filtro condicional `WHERE` para 'PRESENTE', agrupando os participantes e, usando a cláusula `HAVING`, exibe apenas quem esteve em no mínimo 2 atividades.
...
*   **Justificativa/Montagem:** Relaciona as associações dos participantes e os contabiliza atrelados a cada projeto.
...
**10. Crescimento Acumulado de Projetos Mensal (Month-over-month)**
*   **Justificativa/Montagem:** Agrupamentos complexos de tempo combinados com função de data (`DATE_TRUNC`). Adicionalmente, computa a soma móvel com uma Window function sobre todo o horizonte do relatório.
...
**16. Distribuição das Especialidades com Maiores Ofertas (>1 Atividade)**
*   **Justificativa/Montagem:** Usa o aninhamento onde uma Subquery extrai e lista para um comparador `IN` apenas quais especialidades superam a barreira mínima exigida.
*   **Tabelas Consultadas:** `TB_INSTRUTOR`, `RL_ALOCACAO_INSTRUTOR`, `TB_ATIVIDADE`
...
### 4.7. Window Functions (OVER, RANK, SUM OVER, AVG OVER, FIRST_VALUE)
...
*   **Por que foi utilizado e Onde aparecem:** Essenciais para as **Pesquisas Avançadas** deste projeto. Eles foram selecionados para criar cálculos analíticos e evitar o peso de "Auto-Joins" engessados:
    ...
    *   **`SUM(...) OVER(...)`**: Calcula o crescimento acumulado (saldo contínuo/running total) ao longo do tempo ou sequência (Utilizado nas **Avançadas 5 e 10**).
