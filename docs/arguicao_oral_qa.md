...
**Q59 (Correção de Bug: Permissões de Backup): Durante a revisão dos nossos scripts DCL, nós notamos e corrigimos um erro onde o usuário de backup (`dbbackup_ic`) tinha seus privilégios revogados (`REVOKE`) nas tabelas de auditoria. Qual era o risco prático se tivéssemos deixado esse erro passar?**
...
**Q65 (Correção de Bug: Prefixo Reservado no Banco): Durante a execução dos scripts, ocorreu o erro `ERROR: role name "pg_dbbackup" is reserved`. O que causou isso e como resolvemos mantendo o padrão do projeto?**
**Resposta Esperada:** Esse erro foi causado porque o PostgreSQL bloqueia a criação de usuários/roles que comecem com o prefixo `pg_`, reservando isso para as funções internas e contas de administração do próprio sistema de banco de dados. Nós corrigimos a falha renomeando o operador de backup para `dbbackup_ic`. Dessa forma, nós contornamos a restrição do PostgreSQL e passamos a seguir consistentemente o padrão visual de nomenclatura do nosso projeto, que padronizou a criação de perfis com o sufixo `_ic` (como `dba_ic`, `sistema_ic` e `analise_ic`).

**Q66 (Correção de Bug: Violação de Unique Constraint no Povoamento): Durante a execução do script de povoamento de dados, ocorreu um erro informando que duplicar o valor da chave violava a restrição de unicidade "uk_inscricao_unica". Por que isso aconteceu e como resolvemos?**
**Resposta Esperada:** Nós criamos a restrição `UNIQUE (ID_PARTICIPANTE, ID_ATIVIDADE)` para impedir que uma mesma pessoa fosse matriculada duas vezes na mesma atividade. Contudo, nosso script de inserção em massa usava geração de números aleatórios para simular matrículas em larga escala. Estatisticamente, o "sorteio" tentou matricular a mesma pessoa na mesma atividade repetidas vezes, o que acionou corretamente a defesa de consistência do banco de dados e abortou o script. A solução foi adicionar a instrução `ON CONFLICT DO NOTHING` no bloco de inserção da tabela `RL_INSCRICAO_HISTORICO`. Assim, se o gerador aleatório esbarrar em uma restrição de duplicidade, o PostgreSQL ignora o conflito silenciosamente e continua populando o restante dos dados sem quebrar o processo.

**Q67 (Correção de Bug: Omissão de Dados no Agrupamento da Query 5): A nossa Query 5 tinha o objetivo de listar os participantes com 2 ou mais presenças e "mostrar os nomes das atividades". Contudo, o agrupamento (`GROUP BY`) estava escondendo os nomes das atividades e exibindo apenas a contagem bruta. Como corrigimos a query para exibir a lista de nomes sem quebrar o agrupamento?**
**Resposta Esperada:** Quando utilizamos a cláusula `GROUP BY` para agrupar os dados por participante, nós perdemos o acesso direto aos dados individuais das linhas que foram agrupadas (como os títulos únicos das atividades), restando apenas os resultados das funções matemáticas (como o `COUNT`). Para resolver essa limitação e honrar o requisito do relatório, nós introduzimos a função de agregação de texto `STRING_AGG(a.DS_TITULO_ATIVIDADE, ', ')` na cláusula `SELECT`. Essa função varre as linhas de cada participante agrupado, coleta os títulos das atividades e os concatena em uma única *string* textual separada por vírgulas. Isso nos permitiu exibir a lista completa de atividades em uma única coluna sem desfazer a lógica do `GROUP BY`.

**Q68 (Correção de Bug: Omissão de Nomes na Query 10): Assim como na Query 5, a nossa Query 10 precisava listar o total de membros por projeto de extensão e exibir os nomes desses participantes. No entanto, o SQL listava apenas o nome do projeto e a contagem. O que estava errado e qual foi a solução?**
**Resposta Esperada:** O erro possuía exatamente a mesma raiz técnica da Query 5. Ao aplicar o `GROUP BY proj.DS_NOME_PROJETO` para calcular o total numérico de membros (`COUNT`), o motor do banco de dados agregava os participantes em um único bloco opaco, impossibilitando a exibição individual da coluna `p.DS_NOME_PARTICIPANTE` no `SELECT`. A solução seguiu a mesma estratégia: aplicamos a função agregadora de string `STRING_AGG(p.DS_NOME_PARTICIPANTE, ', ')` ao `SELECT`. Com isso, o SGBD extrai os nomes de todos os participantes vinculados àquele agrupamento específico e os retorna como um texto único formatado como lista separada por vírgulas, entregando exatamente o resultado que o relatório pedia.

**Q69 (Correção de Bug: Ausência de Variação Mês-a-Mês na Query Analítica): Uma das nossas queries avançadas prometia exibir o crescimento "mês a mês" de projetos e inscrições, contudo o SQL estava apenas exibindo o valor acumulado ao longo do tempo. Como nós corrigimos a query para extrair matematicamente a variação mensal sem recorrer à aplicação backend?**
**Resposta Esperada:** Originalmente a nossa query utilizava apenas a *Window Function* `SUM(COUNT(...)) OVER(...)` para gerar o montante total acumulado até cada mês do calendário. Para extrair a variação real (o crescimento de um mês em relação ao seu antecessor imediato) via SQL puro, nós inserimos uma nova coluna calculada utilizando a *Window Function* `LAG`. A instrução subtrai o `COUNT` do mês atual pelo resultado de `LAG(COUNT(...)) OVER(...)`, o qual "olha para trás" e captura dinamicamente o valor da linha temporal anterior. Assim o SGBD processa a variação estatística de forma autônoma.
